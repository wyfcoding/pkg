...
type Config struct {
	Version        string               `mapstructure:"version" toml:"version"`               // 配置文件版本号
	Server         ServerConfig         `mapstructure:"server" toml:"server"`                 // 服务器基础配置（HTTP/gRPC）
	Data           DataConfig           `mapstructure:"data" toml:"data"`                     // 数据源配置（DB/Redis/MQ等）
...
type KafkaConfig struct {
	...
	MinBytes       int           `mapstructure:"min_bytes" toml:"min_bytes"`             // 消费者最小抓取字节数
	MaxBytes       int           `mapstructure:"max_bytes" toml:"max_bytes"`             // 消费者最大抓取字节数
...
type TracingConfig struct {
	...
	ServiceName  string `mapstructure:"service_name" toml:"service_name"`   // 注册到追踪系统的服务名称
...
type RateLimitConfig struct {
	...
	Burst   int  `mapstructure:"burst" toml:"burst"`     // 允许的最大突发请求量
}

// CircuitBreakerConfig 定义熔断器（如 Gobreaker）的保护策略
type CircuitBreakerConfig struct {
	Enabled     bool          `mapstructure:"enabled" toml:"enabled"`           // 是否启用熔断器
	MaxRequests uint32        `mapstructure:"max_requests" toml:"max_requests"` // 半开状态允许的最大请求数
...
type ClickHouseConfig struct {
	...
	Username string `mapstructure:"username" toml:"username"` // 登录用户名
...
type Neo4jConfig struct {
	...
	Username string `mapstructure:"username" toml:"username"` // 用户名
...
type ElasticsearchConfig struct {
	...
	Username  string   `mapstructure:"username" toml:"username"`   // 访问用户名

...
var (
	// ErrInvalidExpectedElements 预估容量必须大于 0.
	ErrInvalidExpectedElements = errors.New("expected elements must be greater than 0")
	// ErrInvalidFalsePositiveRate 误报率必须在 (0, 1) 之间.
	ErrInvalidFalsePositiveRate = errors.New("false positive rate must be in range (0, 1)")
)
...
func NewBloomFilter(expectedElements uint, falsePositiveRate float64) (*BloomFilter, error) {
	if expectedElements == 0 {
		return nil, ErrInvalidExpectedElements
	}
